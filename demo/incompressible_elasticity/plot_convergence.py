...
df_bw_P3_P1 = pd.read_pickle('output/bank-weiser_P3_P1/results.pkl')
df_bw_P3_P2 = pd.read_pickle('output/bank-weiser_P3_P2/results.pkl')
df_bw_P4_P2 = pd.read_pickle('output/bank-weiser_P4_P2/results.pkl')
...
y = np.log(df_bw_P4_P2['error_bw'].values[-5:])
A = np.vstack([x, np.ones(len(x))]).T
m, c = np.linalg.lstsq(A, y)[0]
print('BW P4\P2 slope =', m)
...
marker_x, marker_y = marker(df_bw_P3_P1["num_dofs"].values, [df_bw_P3_P1['exact_error'].values, df_bw_P3_P1["error_bw"].values, df_bw_P3_P2["error_bw"].values, df_bw_P4_P2["error_bw"].values, df_bw_P3_P1["error_res"].values], 0.4, 0.05)
...
bw_eff_P4_P2 = np.divide(df_bw_P4_P2['error_bw'].values, df_bw_P4_P2['exact_error'].values)
...
plt.plot(steps, bw_eff_P3_P2, '^-', label=r"$\frac{||\nabla(u - u_h)||}{\eta_{\mathrm{bw}}}$ (P3\P2)", color=dark_map[1])
plt.plot(steps, bw_eff_P4_P2, '^-', label=r"$\frac{||\nabla(u - u_h)||}{\eta_{\mathrm{bw}}}$ (P4\P2)", color=dark_map[2])
