...
plt.loglog(df_bw["num_dofs"], df_bw['error_res'], '^-',
           label=r"$\eta_{\mathrm{res}}$", color=dark_map[1])
...
try:
    plt.loglog(df_bw["num_dofs"], df_bw['error_zz'], '^-',
            label=r"$\eta_{\mathrm{zz}}$", color=dark_map[6])
except:
    pass
...
try:
    marker_x, marker_y = marker(df_bw["num_dofs"].values, [df_bw['error_ver'].values, df_bw['error_res'].values, df_bw['error_zz'].values, df_bw['exact_error'].values, df_bw["error_bw"].values, df_bw["error_res"].values], 0.4, 0.1)
except:
    marker_x, marker_y = marker(df_bw["num_dofs"].values, [df_bw['error_ver'].values, df_bw['error_res'].values, df_bw['exact_error'].values, df_bw["error_bw"].values, df_bw["error_res"].values], 0.4, 0.1)
...
ver_eff = np.divide(df_bw['error_ver'].values, df_bw['exact_error'].values)
res_eff = np.divide(df_bw['error_res'].values, df_bw['exact_error'].values)
try:
    zz_eff = np.divide(df_bw['error_zz'].values, df_bw['exact_error'].values)
except:
    pass
...
try:
    plt.plot(steps, zz_eff, '^-', label=r"$\frac{||\nabla(u - u_h)||}{\eta_{\mathrm{zz}}}$", color=dark_map[6])
except:
    pass
