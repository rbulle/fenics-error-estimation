...
plt.loglog(df_bw["num_dofs"], df_bw['error_zz'], '^-',
           label=r"$\eta_{\mathrm{zz}}$", color=dark_map[8])
...
marker_x, marker_y = marker(df_bw["num_dofs"].values, [df_bw['error_res'].values, df_bw['error_zz'].values, df_bw['error_exact'].values, df_bw["error_bw"].values, df_bw["error_res"].values], 0.4, 0.05)
...
plt.plot(steps, zz_eff, '^-', label=r"$\frac{||\nabla(u - u_h)||}{\eta_{\mathrm{res}}}$", color=dark_map[8])
