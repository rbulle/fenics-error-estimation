...
def main():
    ...
    for i in range(0, 14):
        ...
        eta_bw = bw_estimate(u_h, f, df=k+1, dg=k+1, dof_list=[0, 1, 2, 12, 13, 14])
        ...
        eta_bw_m = bw_estimate(u_h, f, df=k+1, dg=k+1, dof_list= [3, 4, 5, 6, 7, 8, 9, 10, 11])
...
def pbm_data(mesh):
    N = 10          # Number of periods on the diagonal
    coef = 1.       # Coefficient height of the peaks
    alpha = N*sqrt(2.)*np.pi
