'''
Convergence rates comparison between uniform refinement and adaptive refinement
with driven by BW or by the residual estimator for a
unit square domain with uniform Dirichlet BC and an exact solution describing a
...
# Mesh
mesh = Mesh()
with XDMFFile(MPI.comm_world, './mesh/centered_square.xdmf') as f:
    f.read(mesh)
...
r, theta = cartesian2polar_ufl(x, principal_value=False)

rho = 0.25    # Radius of internal layer
u_exact = ufl.atan(60.*(r**2 - rho))
...
# Marking parameter
params['marking parameter'] = 0.95

# Tolerance
params['tolerance'] = 1.e-3

# Iteration max
params['iteration max'] = 50
...
names = ['Err']
est_methods = [None]
mark_methods = [exact_error_marking]
degs = [None]
osc_methods = [None]
'''
results = []
...
for r, res in enumerate(results):
    ...
    df.to_latex('outputs/{}_data_frame.tex'.format(names[r]))

'''
# Plots
convergence_single_plot(names, k, results, rate=-0.5,
                        monochrome=False, path='.')

convergence_multiple_plots(names, k, results, rate = -0.5,
                           monochrome=False, path='.')

efficiency(names, k, results, monochrome=False, path='.')
'''
